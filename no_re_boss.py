...
def Compute(df, discount=1, ESS=None): 
    R=df.corr().astype(np.float32).to_numpy()
    data = df_to_data(df)
    nodes = data.getVariables()
    ss=int(len(df))
    pdis=discount
    starts=1 
    dag = boss(R, ss, pdis, starts)
    return construct_graph(dag, nodes)

def main(folderpath):
    if folderpath.endswith(".csv"):
        base=os.path.dirname(folderpath)
        outputdir=os.path.join(base,"No_resam_Boss")
        ...
        for type in types:
            ...
            if type=='None_PD1':
                ...
                df_90,_,_=result_90
                R=df_90.corr().astype(np.float32).to_numpy()
        ...
        output_dir = os.path.join(folderpath, "No_resam_Boss")
        #print(output_dir)
