#Credit goes to Joe Ramsey from CMU for this code see pytetrad/tools/visualize.py on github. 
...
def write_gdot(gdot, probs, threshold=0, weight=1, length=1, power=1, hidden=lambda pair: False):
    for node in set([key[0] for key in probs] + [key[1] for key in probs]):
        gdot.node(node,
                  ...
                  color="lightgray",
                  stroke="transparent")
        gdot.attr(overlap="false", sep="0.8", nodesep="1", size="10")
    ...
    for pair in probs:
        ...
        for edge in probs[pair]:
            ...
            if marks[0] == "empty" and marks[1] == "empty": 
                color = "#ff0000"
            else: 
                color = "#0000ff"
